...
out_dir <- file.path(Sys.getenv('img_dir'), 'figS5')
...
```{r}
mk_hm <- function(
  M = tar_read(M_cpm_mouse),
  sa = sample_annotation,
  experiment = '') {

  genes = c(
      c('Ifngr1', 'Ifngr2', 'Jak1', 'Jak2', 'Stat1', 'Stat2', 'Irf9'),
      gene_sets[['IFNy']],
     'Tgfbr1', 'Tgfbr2', 'Smad2', 'Smad3', 'Smad4', 
      gene_sets[['TGFb']],
      c('Tnfsr1', 'Tnfsr2', '1l1b', 'Traf1', 'Traf2', 'Rip', 'Nfkb1'),
      gene_sets[['TNFa']]
  )

  geneset_labels = tibble(
    gene = genes, 
    gs = c(
      rep('IFNy pathway', 7), 
      rep('IFNy responsive', length(gene_sets[['IFNy']])),
      rep('TGFb pathway', 5), 
      rep('TGFb responsive', length(gene_sets[['TGFb']])),
      rep('TNFa pathway', 7), 
      rep('TNFa responsive', length(gene_sets[['TNFa']]))
    )
  )

  lM <- subset_feats(M, genes)

  sa <- order_duration(sa)
  if ('m_sg2' %in% colnames(sa) && is.factor(sa$m_sg2)) {
    # browser()
    # sa$m_sg2 <- order_stim_group(sa$m_sg2)
    # levels(sa$m_sg2)
  ...
  if (F) {
    sa <- sa %>%
      dplyr::arrange(m_sg2, duration) %>%
      # order_stim_group() %>%
      # set_rownames(colnames(lM)) %>%
      dplyr::rename(m_sg = m_sg2)
    lM <- lM[, match(colnames(lM), sa$sample_name)]
    sa$m_sg <- droplevels(sa$m_sg)
  }

  sa <- sa[, intersect(c('duration', 'duration2', 'm_sg', 'm_sg2'),
    colnames(sa)), drop = F]
  ...
  pargs <- list(
    ...
    highlight_genes = c(
      'Ifngr1', 'Ifngr2', 'Jak1', 'Jak2', 'Stat1', 'Stat2', 'Irf9',
      'Tgfbr1', 'Tgfbr2', 'Smad2', 'Smad3', 'Smad4', 
      'Tnfsr1', 'Tnfsr2', '1l1b', 'Traf1', 'Traf2', 'Rip', 'Nfkb1'
    ),
    ...
    scale_data = 'none',
    color_name = 'log2FC compared to unstim',
  ...
  set.seed(43)
  DE <- do.call(GE_vs_stim_dur.matrix, args=pargs)
  return(DE)

}

h1 <- mk_hm(M=M_corr_5892, sa=sa_corr_5892)
h2 <- mk_hm(M=M_corr_7296, sa=sa_corr_7296)
...
print_plot_eval({
  ...
  draw(h1, heatmap_legend_side = 'bottom', newpage=FALSE)
  popViewport()
  pushViewport(viewport(
    x = unit(.75, 'npc'), 
    width = unit(.5, 'npc')
  ))
  draw(h2, heatmap_legend_side = 'bottom', newpage=FALSE)
  popViewport()
}, width=17.4, height=25, filename = file.path(l_img_dir,
  glue::glue('heatmap_all_sets.pdf')))
```
