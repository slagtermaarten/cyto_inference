...
HM_col_funs <- list(
  ...
  'experiment' = function(sa) {
     col <- maartenutils::gen_color_vector(
       e_levels(sa$experiment, F),
       name = 'GrandBudapest1')
     col
  },
  ...
  'stim_group' = function(sa) {
    defensive_stim_col_scale(sa$stim_group)
  },
  'm_sg' = function(sa) {
    human_cols <- defensive_stim_col_scale(sa$m_sg)
    new_cols <- setdiff(levels(sa$m_sg), names(human_cols))
    out <- c(human_cols,
  ...
  'experiment' = function(sa)
    maartenutils::gen_color_vector(
      e_levels(sa$experiment, F), 'GrandBudapest1'),
...
#' Plots the output of summarize_pred_error
#'
#'
plot_prediction_error_mat.SummarizedExperiment <- function(
  error_obj,
  plot_id = '',
  cluster_rows = FALSE,
  slice_labels = NULL,
  ...
  error_mat <- assay(error_obj, 'mean')
  colnames(error_mat) <-
    stringr::str_replace(colnames(error_mat), '_error_mean', '')
  colnames(error_mat) <- change_colnames(colnames(error_mat))

  plot_CV <- 'CV' %in% names(assays(error_obj))
  if (plot_CV) {
    CV_mat <- assay(error_obj, 'CV')
    CV_mat[is.na(CV_mat)] <- 0
    colnames(CV_mat) <- change_colnames(colnames(CV_mat))
    CV_mat_inv <- as.matrix(1/CV_mat)
    min_CV_inv <- min(CV_mat_inv[is.finite(CV_mat_inv)])
    max_CV_inv <- max(CV_mat_inv[is.finite(CV_mat_inv)])
  }

  if (cluster_rows) {
    od <- hclust(dist(error_mat))$order
  } else {
    od <- 1:nrow(error_mat)
  }
  error_mat <- error_mat[od, ]
  if (plot_CV) {
    CV_mat <- CV_mat[od, ]
  }

  col_fun <- circlize::colorRamp2(c(0, 1), c('white', 'indianred3'))
  col_fun <- gen_col_fun(matrix(c(0, 1)))

  set.seed(42)

  TA <- HeatmapAnnotation(
    # 'Column mean' = anno_barplot(apply(error_mat, 2, mean)),
    ' ' = anno_boxplot(error_mat, outline = F,
      axis_param = list(gp = gpar(fontsize = heatmap_fs))),
      # gp = gpar(fontsize = heatmap_fs)),
    # foo = anno_block(gp = gpar(fill = 2:7), labels = LETTERS[1:6]),
    # foo = anno_block(
    #   gp = gpar(fill = 'grey95'),
    #   labels_gp = gpar(fontsize = heatmap_fs),
    #   labels = split_labels
  ...
  HM <- Heatmap(
    ...
    width = ncol(error_mat)*unit(10, 'mm'),
    height = nrow(error_mat)*unit(5, 'mm'),
    cell_fun = function(j, i, x, y, width, height, fill) {
      grid.rect(x = x, y = y, width = width, height = height,
        gp = gpar(col = 'grey99', fill = NA))
      # if (error_mat[[i, j]] > 0) browser()
      if (plot_CV && CV_mat[[i, j]] > 0) {
        r_norm <-
          (CV_mat[[i, j]]^-1 - max_CV_inv) /
          (min_CV_inv - max_CV_inv)
      } else {
        r_norm <- 1
      }
      if (F) {
        grid.circle(
          x = x, y = y,
          r = r_norm * min(unit.c(width, height)),
          gp = gpar(fill = col_fun(error_mat[i, j]), col = NA)
        )
      } else {
        grid.roundrect(
          x = x, y = y,
      ...
      if (!is.na(error_mat[i, j]) && error_mat[i, j] > .5) {
        grid.text(round(error_mat[i, j], 2),
          x = x, y = y, gp = gpar(fontsize = 6, col = 'grey10'))
      } else if (!is.na(error_mat[i, j]) && error_mat[i, j] < .5) {
        grid.text(round(error_mat[i, j], 2),
          x = x, y = y, gp = gpar(fontsize = 6, col = 'grey90'))
      }
    },
    cluster_rows = FALSE, 
    ...
    column_title = NULL,
    row_title = 'Experimental conditions'
  )

  RA <- rowData(error_obj) %>%
    { .[od, ] } %>%
    as.data.frame() %>%
    gen_sample_annotation_HM(
      heatmap_type = 'row',
  ...
  o_fn <- file.path(
    out_dir,
    glue::glue('error_matrix{prepend_hyphen(plot_id)}.pdf')
  )
  p_width <- ncol(error_mat) * 1 + 6
  print_plot_eval(
    {
      draw(HM + RA, merge_legend = T)
    },
    height = 15,
    width = p_width,
    filename = o_fn
...
#'
#'
plot_prediction_error_mat.data.frame <- function(
  error_obj,
  plot_id = '',
  cluster_rows = F,
  slice_labels = NULL,
  ...
  min_CV_inv <- min(CV_mat_inv[is.finite(CV_mat_inv)])
  max_CV_inv <- max(CV_mat_inv[is.finite(CV_mat_inv)])
  # if (!is.finite(min_CV_inv)) min_CV_inv <- 0
  # if (!is.finite(max_CV_inv)) min_CV_inv <- 1
  ...
  HM <- Heatmap(
    ...
    cell_fun = function(j, i, x, y, width, height, fill) {
      ...
      grid.circle(
        x = x, y = y,
        r = r_norm * .5 * min(unit.c(width, height)),
        gp = gpar(fill = col_fun(error_mat[i, j]), col = NA)
      )
      grid.text(round(error_mat[i, j], 2),
        x = x, y = y, gp = gpar(fontsize = 6))
    },
  ...
  RA <- error_obj %>%
    dplyr::select(experiment, stim_group, duration) %>%
    { .[od, ] } %>%
    as.data.frame() %>%
    gen_sample_annotation_HM(
      heatmap_type = 'row',
...
extract_sa <- function(...) UseMethod('extract_sa')


extract_sa.Seurat <- function(so,
  meta_fields = c('stim_group', 'duration')) {
  sa <- extract_sa(so@meta.data, meta_fields = meta_fields)
  return(sa)
}


extract_sa.data.frame <- function(dtf,
  meta_fields = c('stim_group', 'duration')) {
  sa <- dtf %>%
    dplyr::select(any_of(meta_fields)) %>%
    order_stim_group() %>%
    order_concentration() %>%
    order_duration() %>%
    # numerify_regressors()
...
GE_vs_stim_dur.matrix <- function(
  ...
  if (T) {
    ...
    if (scale_data == 'Z') {
      M <- t(scale(t(M)))
      max_Z = 5
      message('Capping ', sum(abs(M) > max_Z), ' entries to ', max_Z)
      M[abs(M) > 5] <- max_Z
    }
  ...
  if (scale_data == 'Z') {
    vn <- paste0(ifelse(scale_data == 'Z', 'Z-scaled\n', ''),
        'gene expression') %>%
        maartenutils::simple_cap(cap_first_word_only = T)
  } else if (scale_data == 'robust') {
    vn <- paste0(ifelse(scale_data, 'Robust-scaled\n', ''),
        'gene expression') %>%
        maartenutils::simple_cap(cap_first_word_only = T)
  } else {
    vn <- 'Gene expression'
