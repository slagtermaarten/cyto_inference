...
filtering_funs <- list(
  'basic' = function(obj, min_frac_Nhoods_retained = .21) {
    tryCatch({
      obj %>%
        as_tibble() %>%
...
# sort_vars = c('ifn_conc_IZ', 'duration_IZ', 'tnf_conc_IZ',
#   'mean_error', NULL)
rank_weights <- list(
  'TNFa_CI' =
    list(
...
rank_funs <- map(rank_weights, function(penalty_weights) {
  ...
  out <- function(obj) {
    penalty <-
      ...
      rowMeans() %>%
      { . }
    exp(-penalty_weights$frac_Nhoods_retained_p * obj$frac_Nhoods_retained) * penalty
  }
...
sort_bm_res <- function(
  ...
  if ('gamma_D' %in% colnames(bm_res) && !'ltb' %in% colnames(bm_res)) {
  ...
  #     browser()
  #     class(x)
  #     if ('gamma_D' %in% colnames(x) && !'ltb' %in% colnames(x)) {
  #       x <- dplyr::rename(x, ltb = gamma_D)
  #     }
  #     return(x)
  #   })
  ...
  if (F) {
    scores[entry_ord]
    # cor(entry_ord, bm_res$mean_error[entry_ord], method = 'spearman')
    # cor(entry_ord, bm_res$mean_error, method = 'spearman')
    # cor(1:length(entry_ord), bm_res$mean_error[entry_ord], method = 'spearman')
...
summarize_bm_res <- function(
  ...
  mod_names = FALSE,
  # sort_vars = c('duration_IZ', 'tnf_conc_IZ', 'mean_error',
  #   'ifn_conc_IZ', NULL),
  filtering_fun = filtering_funs[[1]],
  ...
  # table(bm_res_flat$kpca); table(bm_res_flat$name)
  ...
  if (F) {
    sort_inverse_vars <-
      stringr::str_subset(sort_vars, 'IZ|error')
    bm_res_flat <-
      bm_res_flat %>%
      dplyr::mutate(across(any_of(sort_inverse_vars), ~-1*.x)) %>%
      dplyr::arrange(across(any_of(sort_vars))) %>%
...
DISTINCT <- function(
  ...
  if (stringr::str_detect(obj$hyperparams$tier1, 'kpca')) {
    obj$tier1_settings <- subset_grid_dtf(obj$hyperparams$tier1, 
      grid_dtf = NH_kpca_grid, 
      remove_obj_refs = F)
  } else {
    obj$tier1_settings <- subset_grid_dtf(obj$hyperparams$tier1, 
      grid_dtf = NH_t_grid,
...
as_nrs.DISTINCT <- function(
  ...
  nrs <- NhoodRefSim$new(
    ...
    exclusion_ref_samples = exclusion_ref_samples,
    ref_sa =
...
hl_settings_heatmap <- function(
  ...
  sort_vars <- intersect(sort_vars, colnames(bm_res_sum))
  ...
  settings_comp <-
    settings_comp[, which(
      imap_lgl(settings_comp,
        ~length(unique(.x)) > 1 | .y %in% sort_vars | .y %in% vis_vars)
    )]
  ...
  iz_anns <-
    purrr::map(c(sort_vars, vis_vars), function(x) {
      largs <- list(
  ...
  print_plot_eval(
    {
      ...
      eps <- .5
      for (vn in c(sort_vars, vis_vars)) {
...
settings_scatter.DISTINCT_bm_summary <- function(bm_res_sum, 
  ...
  plots <-
    ...
    pmap(function(x_var, y_var, type) {
      if (type == 'scatter') {
        p <- 
          bm_res_sum %>%
