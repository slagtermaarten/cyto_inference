...
recover_garbled_string <- function(
  query, candidates, N_return = 1L,
  sim_f = RecordLinkage::levenshteinSim) {
  if (!requireNamespace('RecordLinkage')) {
    install.packages('RecordLinkage')
  }
  # pacman::p_load('RecordLinkage')
  N_return <- min(N_return, length(candidates))
  out <- maartenutils::auto_name(query) %>%
    purrr::map(
      ~candidates[base::order(sim_f(.x, candidates), decreasing = TRUE)[1:N_return]]
    )
  return(out)
}
...
find_sa_match_col <- function(v, dtf = sample_annotation) {
